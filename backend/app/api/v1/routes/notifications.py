...
@router.get("")
def list_notifications_endpoint(db: Session = Depends(get_db)) -> list[Notification]:
    """Undismissed and unresolved - the Notifications panel (§8.1 item 5)."""
    return list(service.list_active(db))


@router.post("/{notification_id}/dismiss")
def dismiss_notification_endpoint(notification_id: str, db: Session = Depends(get_db)) -> Notification:
    try:
