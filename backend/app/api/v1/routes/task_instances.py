...
def complete_endpoint(
    instance_id: str, db: Session = DB_SESSION, jobs: JobScheduler = Depends(get_request_job_scheduler)
) -> TaskInstance:
    try:
...
def start_endpoint(instance_id: str, db: Session = DB_SESSION) -> TaskInstance:
