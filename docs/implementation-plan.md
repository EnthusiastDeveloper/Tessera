...
| A fixed task displaces flexible work instead of being blocked by it (design doc Rev 10: §6.5, §11 item 12; architecture-plan Rev 4 §4.1) | Design doc Rev 10 | **Not started** | - | Today a scheduled flexible instance counts as a §6.5 conflict, so creating a fixed task on its slot fails with `creation_conflict`. Drop scheduled flexible instances from the fixed-conflict obstacle set and have every fixed-slot path re-place the flexible instances it lands on. Worked Example A's flexible variant is the acceptance fixture. |
| A fixed task waiting on a dependency (design doc Rev 10: §4, §6.5, §6.6, §6.9, §11 item 13; IRR-2 H11) | Design doc Rev 10 | **Not started** | - | Blocked fixed instances hold their slot (obstacle + conflict + sync_conflict), get reminder/overdue jobs from creation, go overdue normally with the prerequisite named, and become `scheduled` - not `pending` - on unblock (also fixes the stuck-in-`pending` bug). Worked Example N2 is the acceptance fixture. |
