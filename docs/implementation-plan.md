...
| 8 | API Hardening & Backend E2E | **Done** | `stage-08-api-hardening` | Error-envelope/OpenAPI/throttling audit, plus three real contract gaps found and fixed (confirmed with the user before building each): `dismiss`, scoped `DELETE`, and the previously-nonexistent `GET /task-instances` list/backlog endpoint. `app.scheduling.orchestration` gained `archive_template_and_cancel_jobs`, shared by `app.task_templates.service.archive_template` and `app.task_instances.service.delete_instance`'s `this_and_future` scope (independent siblings, can't call each other directly). **Deliberately deferred to its own follow-up PR:** architecture-plan §5.1's expected-values PATCH optimistic-locking mechanism - large and design-sensitive enough to warrant a focused PR of its own. 47 new tests. See the Stage 8 section below for full detail |
...
**Exit criteria**
- [x] Full backend suite (all stages) green in one CI run - 454 passing (47 new this stage).
