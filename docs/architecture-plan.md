...
- **(Added Rev 2, design doc 3.10)** Edit scope is an explicit, required param, not inferred:
  ...
  - `PATCH /task-templates/{id}?scope=this_and_future&from_instance={instance_id}` - a **"this and future"** edit (design doc 3.10, rewritten Rev 10). Writes the template and, in the same transaction, the matching fields on the `from_instance` occurrence and on every open occurrence of the series with a later `nominal_date`; any flexible one whose placement is invalidated re-enters 6.2. `from_instance` is **required** - without it "this" is undefined - and must belong to the template and be non-terminal.
...
- **(Added Rev 2, design doc 3.10; widened Rev 4)** A "this and future" `TaskTemplate` edit is a **separate trigger**, not a variant of the instance-mutation methods above. Its service method must: (1) write the template, (2) collect the reached occurrences - the `from_instance` occurrence plus every non-terminal occurrence of the series with a later `nominal_date`, minus the `detached` ones unless `include_detached` - (3) validate all of them before writing any, (4) apply the matching fields to each (clearing `detached` where design doc 3.10 says to), and (5) for each one whose change was job-relevant (`scheduled_time`, `reminder_offsets_minutes`-derived fire times, `deadline`), re-wire its jobs exactly as the direct-mutation methods would. Revision 2's version touched one instance; the loop is new, and a loop that validates and writes one occurrence at a time would leave a half-applied edit behind when a later occurrence fails. This remains the path most likely to be half-implemented by accident - call it out explicitly in code review, not just here.
- **(Added Rev 4, design doc 6.5) A fixed task landing on a slot displaces flexible work there.** Every path that gives a fixed instance a new or longer slot - create, generation at an occurrence boundary, reschedule, a this-occurrence duration change, a this-and-future retime or duration change - must, in the same method and after writing the fixed instance, return each overlapping `scheduled` flexible instance to `pending` and place it again, re-wiring its reminder/overdue jobs and raising `unschedulable` if nothing fits. One shared helper, called from each path, rather than a copy per path; the §6.5 conflict check itself no longer counts scheduled flexible instances as obstacles.
- **(Added Rev 4, design doc 6.5/6.6/6.9) A blocked fixed instance is wired like a scheduled one.** It holds its slot, so it is in the obstacle and conflict sets, and it gets reminder and overdue jobs from creation (the overdue handler accepts it and names the unfinished prerequisite). Unblocking a fixed instance transitions it to `scheduled`, never `pending`, and keeps those jobs; startup reconciliation (4.2) recreates them for blocked fixed instances too instead of cancelling them.
...
| **Job-wiring tests** | Every direct `TaskInstance` mutation path (create/this-occurrence-edit/reschedule/delete/complete/extend-deadline), **plus (Rev 2) the "this and future" template-propagation path** - asserts job-store state directly, per 4.1 | Required for every mutation path, not optional/best-effort. Propagation case needs its own tests: with several open occurrences, a "this and future" edit re-wires the jobs of the edited occurrence and every later non-detached one, leaves earlier ones alone, skips a later `detached` one unless `include_detached`, and - when one occurrence fails validation - writes nothing and changes no job. |
...
| End-to-end tests | Small set of critical flows only: login, create-with-conflict, create-flexible-and-schedule, complete-task, extend-a-missed-deadline, **(added Rev 2)** edit-a-recurring-task-both-scopes-and-verify-detach, **(added Rev 4)** a "this and future" edit that names the skipped occurrence and applies to it once the checkbox is ticked | Not exhaustive - e2e suites get expensive to maintain if over-built |
