...
| 4 | Sync with design doc Revision 10 (recurring-series rules). Section 3: templates take `start_date`; a "this and future" edit names the occurrence it starts from and whether to include individually edited ones; `GET /task-instances` filters by template. Section 4.1: the "this and future" path fans out to every open occurrence from the edited one onward. Section 4.1: fixed-task paths displace overlapping flexible work. Section 8: coverage for both |
